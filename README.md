...
- `GitHub 仓库`：规范维护、完整字体策略、E2E 测试、打包脚本和 issue 跟踪都以 GitHub 为准。
- `Overleaf Gallery`：用于快速预览和引流的轻量社区版，不承载完整资源。

如果你要开始真实写作、提交学校版本、或跟进最新修复，请优先从 GitHub 克隆完整仓库，而不是只使用社区镜像。
...
这套模板按《江苏海洋大学本科校级优秀毕业实习与设计（论文）摘要格式说明》的结构单独组织：首页保留题名、作者、指导教师与中文摘要全宽区块，正文与参考文献切换到双栏，英文标题与英文摘要在文末回到整页全宽。该入口默认启用严格字体检查：缺少正式提交所需字体时会直接报错，不再静默回退开源替代字体。
